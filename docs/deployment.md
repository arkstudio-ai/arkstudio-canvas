...
整个 stack（MySQL + Backend + Web）用 Docker Compose 一把拉起；你只需要装好 Docker 就行。
...
> 默认 web 暴露在 `:8080`；MySQL 默认不对外，要用 GUI 工具时改 `docker-compose.yml` 里 mysql service 加 `ports: ["3306:3306"]`。
...
```bash
docker compose down -v   # ⚠️ 会删 mysql named volume
```
