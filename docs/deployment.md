...
整个 stack（Backend + Web，DB 默认 SQLite 直接落容器卷）用 Docker Compose 一把拉起；你只需要装好 Docker 就行。

> 想跑 MySQL 而不是默认的 SQLite？看 [docs/database-mysql.md](./database-mysql.md)，那里有最小覆写文件示例。
...
1. SQLite 模式（`DATABASE_URL=file:...`）下先 `mkdir -p` 库文件父目录
2. `prisma db push` 把 schema 同步到 DB（`schema.prisma` 是权威源）；MySQL 模式下还会重试 30s 等 server 起来
...
```bash
# 备份（SQLite 默认）—— .backup 会等正在写入的事务完成后做一致快照
...
> 跑 MySQL 而非 SQLite 的备份（`mysqldump`）见 [docs/database-mysql.md](./database-mysql.md)。
...
| 启动后 backend 一直 restart | `docker compose logs backend` 看 `[entrypoint]`；常见原因：SQLite 模式下父目录权限错（容器挂载 `/data/db` 异常）、或 MySQL 模式下 `DATABASE_URL` 写错 / server 未就绪 |
