...
export class TemplatesService {
  ...
  async create(createDto: CreateTemplateDto) {
    ...
    const validNodes = json.nodes.filter((node: any) => {
      ...
      if (flowNode.data === null && flowNode.params === null) return false;
      return true;
    });
    ...
    const transformedData = this.transformCoordinates(
      validNodes,
      json.groups || [],
      json.edges || [],
    );
    ...
    const nodesData = transformedData.nodes.map((node: any) => {
      ...
      return {
        ...
        data: (flowNode?.data ?? Prisma.JsonNull) as Prisma.InputJsonValue,
        params: (flowNode?.params ?? Prisma.JsonNull) as Prisma.InputJsonValue,
      };
    });

    const transformedEdges = transformedData.edges
      .filter(
        (edge: any) => nodeIdMap.has(edge.source) && nodeIdMap.has(edge.target),
      )
      .map((edge: any) => ({
        ...edge,
    ...
    const tagsCreate =
      tags?.map((tag) => ({
        tag: {
          connectOrCreate: {
            where: {
              category_value: { category: tag.category, value: tag.value },
            },
    ...
    return this.prisma.$transaction(async (tx) => {
      if (updateDto.removeTags?.length) {
        for (const tagToRemove of updateDto.removeTags) {
          const tag = await tx.tag.findUnique({
            where: {
              category_value: {
                category: tagToRemove.category,
                value: tagToRemove.value,
              },
            },
          });
          if (tag) {
            await tx.tagsOnTemplates.deleteMany({
              where: { templateId: id, tagId: tag.id },
            });
          }
        }
      }
      ...
      if (updateDto.addTags?.length) {
        for (const tagToAdd of updateDto.addTags) {
          ...
          if (!exists) {
            await tx.tagsOnTemplates.create({
              data: { templateId: id, tagId: tag.id },
            });
          }
        }
      }
  ...
  async instantiate(templateId: string) {
    ...
    const nodes = template.nodes.map((node) => {
      ...
      const structureNode = structure.nodes?.find(
      ...
      return {
        ...
        width: node.width,
        height: node.height,
        groupId: structureNode?.groupId
          ? groupIdMap.get(structureNode.groupId)
          : undefined,
